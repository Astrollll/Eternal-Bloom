...
const PLAYER_DEFAULT_SHOTS_PER_ATTACK: int = 1
const PLAYER_DEFAULT_SHOT_INTERVAL: float = 0.06
const ENEMY_DEFAULT_BURST_SHOTS: int = 3
const ENEMY_DEFAULT_BURST_INTERVAL: float = 0.07

static func shoot_projectile(owner: Node2D, direction: Vector2, mask: int, shots: int = PLAYER_DEFAULT_SHOTS_PER_ATTACK, interval: float = PLAYER_DEFAULT_SHOT_INTERVAL) -> void:
	# Fire one or more player shots toward the nearest enemy.
	_spawn_projectile_burst(owner, direction, mask, false, max(1, shots), max(0.0, interval))

static func shoot_enemy_projectile(owner: Node2D, direction: Vector2, mask: int, shots: int = ENEMY_DEFAULT_BURST_SHOTS, interval: float = ENEMY_DEFAULT_BURST_INTERVAL) -> void:
	# Fire an enemy-style projectile burst using the same projectile scene profile.
	_spawn_projectile_burst(owner, direction, mask, true, max(1, shots), max(0.0, interval))
