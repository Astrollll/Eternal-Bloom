...
@export var attack_fire_interval: float = 0.24
@export_range(1, 6, 1) var projectile_shots_per_attack: int = 1
@export var projectile_shot_interval: float = 0.06
...
@export var knockback_decay_rate: float = 1400.0
...
func _physics_process(delta: float) -> void:
	...
	knockback_velocity = knockback_velocity.move_toward(Vector2.ZERO, knockback_decay_rate * delta)
	...
	if Input.is_action_pressed("attack") and _can_start_attack():
		_start_attack()
...
func _start_attack() -> void:
	...
	attack_cooldown_left = max(0.01, attack_fire_interval)
	is_attacking = true
	velocity = Vector2.ZERO
	...
	if target != null and dist <= melee_trigger_range:
		...
		PlayerAttackModule.shoot_projectile(self , shoot_dir, attack_collision_mask, projectile_shots_per_attack, projectile_shot_interval)
		is_attacking = false

func _can_start_attack() -> bool:
	# Holding attack should repeat attacks, but only when not busy and cooldown has elapsed.
	return not is_attacking and not is_dashing and attack_cooldown_left <= 0.0
