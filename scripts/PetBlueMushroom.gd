...
func _ready() -> void:
	# Pick the correct idle pose and remember the starting vertical position for bobbing.
	anim.play("face_left" if facing_left else "face_right")
	_base_y = anim.position.y

func _process(delta: float) -> void:
	# Add a gentle hover and pulse so the pet feels alive without needing extra animation frames.
	_idle_time += delta
